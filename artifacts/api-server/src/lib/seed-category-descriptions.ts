...
export async function seedCategoryDescriptions(): Promise<number> {
  const entries = Object.entries(staticDescriptions).map(([category, description]) => ({
    category,
    description,
    updatedAt: new Date(),
  }));
  ...
  for (const entry of entries) {
    try {
      ...
      logger.warn({ err, category: entry.category }, "seed: failed to insert category");
    }
  }
  logger.info({ seeded, total: entries.length }, "Seeded category descriptions");
  return seeded;
}
