...
const RSS_ARTWORK_CONCURRENCY = 5;
...
function enrichFeedArtworkInBackground(feed: RssFeed): void {
  const missing = feed.episodes.filter((e) => !e.artworkUrl);
  if (missing.length === 0) return;

  logger.debug({ count: missing.length }, "RSS artwork enrichment: starting background lookup");
  ...
  pMap(
    ...
    RSS_ARTWORK_CONCURRENCY,
  ).then(() => {
    const enriched = feed.episodes.filter((e) => e.artworkUrl).length;
    logger.debug({ enriched, total: feed.episodes.length }, "RSS artwork enrichment: complete");
  }).catch(() => {
