...
import { getFeedCached, getAutoGeneratedDescriptions, getAutoGeneratedDescriptionEntries, type RssEpisode } from "../lib/rss";
...
router.get("/categories", async (_req, res) => {
  try {
    ...
    const autoDescEntries = getAutoGeneratedDescriptionEntries();
    const counts = new Map<string, number>();
    ...
    const list = [...counts.entries()]
      ...
      .map(([name, count]) => {
        ...
        const hasCurated = dbDesc !== null || curated !== null;
        const autoEntry = autoDescEntries.get(name);
        const autoDescriptionStale =
          !hasCurated &&
          autoEntry !== undefined &&
          now - new Date(autoEntry.latestPubDate).getTime() > AUTO_DESC_STALE_MS;

        return { name, count, description, autoDescriptionStale };
      });
    res.json(list);
  } catch (err) {
