        ...
        {isLoading ? (
          ...
          <div className="flex gap-4 overflow-x-auto pb-2 snap-x snap-mandatory -mx-4 px-4 md:mx-0 md:px-0 md:grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 md:overflow-x-visible">
            {[...episodes].sort((a, b) => new Date(b.pubDate).getUTCFullYear() - new Date(a.pubDate).getUTCFullYear()).map((ep) => {
              ...
              const bullets = ep.bulletPoints?.filter(Boolean) ?? [];
              const sources = ep.sourceLinks?.filter(s => s.label && s.url) ?? [];
              const historySegment = ep.historySegment as HistorySegment | undefined;
              ...
              return (
                ...
                >
                  ...
                  {bullets.length > 0 && (
                    <ul className="flex flex-col gap-1 min-h-0">
                      {bullets.slice(0, 3).map((b, i) => (
                        <li key={i} className="flex items-start gap-1.5 min-w-0">
                          <span className="text-[#D9A066]/70 mt-0.5 shrink-0 text-[10px]">▸</span>
                          <span className="text-[11px] text-white/70 leading-tight line-clamp-2">{b}</span>
                        </li>
                      ))}
                    </ul>
                  )}
                  ...
                  <div className="flex flex-col gap-2 mt-auto pt-1">
                    {/* Prefer historySegment lessonText, fallback to lessonQuote */}
                    {historySegment?.lessonText ? (
                      <p className="text-xs text-[#D9A066]/80 italic leading-snug line-clamp-3 border-l border-[#D9A066]/30 pl-2">
                        "{historySegment.lessonText}"
                      </p>
                    ) : ep.lessonQuote ? (
                      <p className="text-xs text-[#D9A066]/80 italic leading-snug line-clamp-3 border-l border-[#D9A066]/30 pl-2">
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        {sources.slice(0, 2).map((s, i) => (
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="inline-flex items-center gap-1 text-[9px] font-medium text-white/50 hover:text-[#D9A066] transition-colors"
                          >
                    ...
                    {historySegment && ep.audioUrl && historySegment.startSeconds > 0 ? (
                      <div className="flex flex-col gap-1.5 mt-1">
                        <span className="text-[10px] font-bold uppercase tracking-widest text-[#D9A066]/60">
                          History Segment
                        </span>
