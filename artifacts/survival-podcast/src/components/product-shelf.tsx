...
export function ProductShelf({ products, heading, subheading, compact = false }: ProductShelfProps) {
  ...
  return (
    <section className={compact ? "" : "py-2"}>
      ...
      <div className="flex gap-3 overflow-x-auto pb-2 snap-x snap-mandatory scrollbar-thin scrollbar-thumb-border">
        {products.map((p) => (
...
export function ProductShelfSection({ products, heading, subheading, zoneColor }: ProductShelfSectionProps) {
  ...
  return (
    <section>
      ...
      <div className="flex gap-3 overflow-x-auto pb-3 snap-x snap-mandatory scrollbar-thin scrollbar-thumb-border ml-9">
        {products.map((p) => (
          <ProductCard key={p.id} product={p} />
        ))}
      </div>
